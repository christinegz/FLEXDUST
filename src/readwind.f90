...
subroutine readwind(indj,n,uuh,vvh,wwh)
  ...
  !
	write(*,*) 'Opening file: ' ,path(3)(1:length(3)) &
         //trim(wfname(indj))
...
!    stop
 ...
 ! write(*,*) isec1(6)
  ...
  !        spurious negative values
      ...
      !CGZ
      !***********************
!      if(isec1(6).eq.140) sw(i,j,1,n)= &!! Soil wetness
!           zsec4(nxfield*(ny-j-1)+i+1)
!      if(isec1(6).eq.140) write(*,*) 'Read soil wetness'
    ...
    !  if(isec1(6).eq.139) write(*,*) 'Read soil temperature'
       ...
       if(isec1(6).eq.27) cvl(i,j,1,n)= &!! Low vegetation cover
           zsec4(nxfield*(ny-j-1)+i+1)
    !    if(isec1(6).eq.27) write(*,*) 'Read low vegetation'
        
       if(isec1(6).eq.28) cvh(i,j,1,n)= &!! High vegetation cover
           zsec4(nxfield*(ny-j-1)+i+1)
    !    if(isec1(6).eq.28) write(*,*) 'Read high vegetation'
       !***********************
...
50   call grib_close_file(ifile)
 ...
 ! write(*,*)'SWITCHED OFF USTAR FROM STRESS (READWIND), use PBL_PROFILE'
