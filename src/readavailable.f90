...
subroutine readavailable(beg, end)
...
700   read(unitavailab,'(i8,1x,i6,2(6x,a255))',end=699) ldat, &
      ...
      if ((jul.ge.beg).and.(jul.le.end)) then
       ...
       !write(*,*) 'nest', beg, end, jul, numbwfn(k)
...
699   continue
  ...
  do i=2,numbwf
    ...
    if (idiff.gt.idiffmax) then
     ...
     print*, 'Diff:', idiff, idiffmax, wftime(i), wftime(i-1), i
  ...
  do k=1,numbnests
    if (numbwfn(k).ne.numbwf) then
      ...
      write(*,*) 'ERROR AT NEST LEVEL: ',k, numbwfn(k), numbwf
      stop
    endif
    do i=1,numbwf
